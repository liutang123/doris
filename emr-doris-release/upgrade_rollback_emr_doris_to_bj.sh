...
#
work_dir_prefix="/data/emr/upgrade_packages_dir"
cos_bucket_url="https://derenli-bj-1301087413.cos.ap-beijing.myqcloud.com"
doris_tar="emr_doris_v1.1.0.tar.gz"
doris_tar_md5="9e5cf05551383d7f6d61d5db468d322c"
jdk_11_tar="java-11.tar.gz"
#
################################################################################################################################
...
upgrade_doris() {
  ...
  chown hadoop:hadoop -R "${dest_dir}"
  log "[INFO] success to upgrade doris to v${new_version}."

  # check and decide to download java11
  dest_dir="/usr/local/java-11"
  if [ ! -d ${dest_dir} ]; then
    log "[INFO] need to download java 11"
    source_file=${jdk_11_tar}
    wget -q ${cos_bucket_url}/${source_file} -P ${sourceDir}
    if [ $? -ne 0 ]; then
      error_on_rollback "wget file ${source_file} failed!"
      exit 1
    fi
    log "[INFO] downloaded ${jdk_11_tar} package to ${sourceDir}."

    untar_work_dir="/usr/local"
    upgrade_file_path="${sourceDir}/${source_file}"
    tar -zxf "${upgrade_file_path}" -C "${untar_work_dir}"
    if [ $? -ne 0 ]; then
      error_on_rollback "unzip tar package ${upgrade_file_path} failed!"
      exit 1
    ...
    log "[INFO] untar ${upgrade_file_path} to ${untar_work_dir} successfully."
    echo "${dest_dir}" >> ${newAddDirList}
  fi
}
