...
export BROKER_LOG_DIR=/data/emr/doris/broker/log
...
echo "the doris broker java home is $doris_broker_java_home" >> $BROKER_LOG_DIR/apache_hdfs_broker.out

if [ "$doris_broker_java_home" = "" ]; then
  echo "Error: doris_broker_java_home is not set." >> $BROKER_LOG_DIR/apache_hdfs_broker.out
  exit 1
fi
