...
import com.tencent.oceanus.shaded.org.apache.flink.table.data.ArrayData;
import com.tencent.oceanus.shaded.org.apache.flink.table.data.MapData;
...
public class SetatsArrayColumnValue implements ColumnValue {
    private static final Logger LOG = LoggerFactory.getLogger(SetatsArrayColumnValue.class);
    private int idx;
    private ArrayData record;
    private ColumnType dorisType;

    public SetatsArrayColumnValue() {
    }

    public SetatsArrayColumnValue(ArrayData record, int idx, ColumnType columnType) {
        this.idx = idx;
        this.record = record;
    ...
    public void unpackArray(List<ColumnValue> values) {
        ArrayData recordArray = record.getArray(idx);
        for (int i = 0; i < recordArray.size(); i++) {
            SetatsArrayColumnValue arrayColumnValue = new SetatsArrayColumnValue(recordArray, i,
    ...
    public void unpackMap(List<ColumnValue> keys, List<ColumnValue> values) {
        MapData map = record.getMap(idx);
        ArrayData key = map.keyArray();
        for (int i = 0; i < key.size(); i++) {
            SetatsArrayColumnValue keyColumnValue = new SetatsArrayColumnValue(key, i,
                    dorisType.getChildTypes().get(0));
            keys.add(keyColumnValue);
        }
        ArrayData value = map.valueArray();
        for (int i = 0; i < value.size(); i++) {
            SetatsArrayColumnValue valueColumnValue = new SetatsArrayColumnValue(value, i,
