...
public class DataMaskPolicyTest extends TestWithFeService {
    ...
    public void testDataMaskForQuery() throws Exception {
        ...
        sql = "create data mask policy test1 on internal.test.table1.k2 to jack using MASK_SHOW_LAST_4";
        executeSql(sql);

        useUser("jack");
        String queryStr = "EXPLAIN select * from test.table1";
        String explainString = getSQLPlanOrErrorMsg(queryStr);
        Assertions.assertTrue(explainString.contains("sha2(CAST(k1[#2] AS varchar(65533)), 256)"));
        ...
        List<List<String>> resultRows = showDataMaskPolicy(null, null).getResultRows();
        Assertions.assertEquals(0, resultRows.size());
    }

    @Test
    public void testDataMaskForPriority() throws Exception {
        setSession("set enable_nereids_planner = true");
        useUser("root");
        grant("GRANT 'role1' TO 'jack'@'%'");

        String sql = "create data mask policy test on internal.test.table1.k2 to role role1 using MASK_REDACT";
        executeSql(sql);

        sql = "create data mask policy test1 on internal.test.table1.k2 to jack using MASK_HASH level 1";
        executeSql(sql);

        useUser("jack");
        String queryStr = "EXPLAIN select * from test.table1";
        String explainString = getSQLPlanOrErrorMsg(queryStr);
        Assertions.assertTrue(explainString.contains("sha2(k2[#1], 256)"));
