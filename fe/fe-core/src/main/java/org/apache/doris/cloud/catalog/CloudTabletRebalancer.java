...
public class CloudTabletRebalancer extends MasterDaemon {
    ...
    public Set<Long> getSnapshotTabletsInPrimaryByBeId(Long beId) {
        ...
        Set<Long> tabletIds = new HashSet<>(
                (null == tablets ? 0 : tablets.size()) + (null == colocateTablets ? 0 : colocateTablets.size())
        );
    ...
    private void batchWarmUp(String clusterId, @Nullable List<InfightTask> tasks) {
        ...
        List<Long> tablets = tasks.stream().map(task -> {
            Preconditions.checkState(task.srcBe == srcBe && task.destBe == destBe,
                    "all tasks should be from same be to same be");
    ...
    private boolean taskIsTimeout(InfightTask task) {
        return task.startTimestamp > 0
                && System.currentTimeMillis() / 1000 - task.startTimestamp > Config.cloud_pre_heating_time_limit_sec;
    }
    ...
    public void checkInflightWarmUpCacheAsync() {
        ...
        for (Map.Entry<Long, List<InfightTask>> entry : beToInfightTasks.entrySet()) {
            ...
            for (Map.Entry<Long, Boolean> result : taskDone.entrySet()) {
                ...
                if (task != null && (result.getValue() || taskIsTimeout(task))) {
                    if (!result.getValue()) {
            ...
            LOG.info("be {} pre cache success/timeout/total {}/{}/{}",
                    entry.getKey(), success, timeout, tablets.size());
        }
        long oldSize = infos.size();
        infos = batchUpdateCloudReplicaInfoEditlogs(infos);
    ...
    private Map<Long, Boolean> sendCheckWarmUpCacheAsyncRpc(List<Long> tabletIds, long be) {
        ...
        try {
            ...
            if (result.getStatus().getStatusCode() != TStatusCode.OK) {
                ...
                LOG.info("check pre {} tablets {}.. cache succ status {} {}", tabletIds.size(),
                        tabletIds.size() <= 10 ? tabletIds : tabletIds.subList(0, 10),
            ...
            if (Config.enable_cloud_warm_up_for_rebalance && srcBackend != null && srcBackend.isAlive()) {
                ...
                if (futureTasks != null) {
                    ...
                    if (tasks.size() >= Config.cloud_warm_up_rpc_batch_size) {
                        List<ForkJoinTask<?>> results = new ArrayList<>();
                        futureTasks.forEach((pair, sendTasks) -> {
                            results.add(warmUpSendRpcPool.submit(() -> batchWarmUp(clusterId, sendTasks)));
                        });
                        for (ForkJoinTask<?> result : results) {
                            result.join();
                        }
