...
public class BackupHandler extends MasterDaemon implements Writable {
    ...
    private void restore(Repository repository, Database db, RestoreStmt stmt) throws DdlException {
        ...
        if (stmt.isLocal()) {
            ...
            restoreJob = new RestoreJob(stmt.getLabel(), backupTimestamp, db.getId(), db.getFullName(), jobInfo,
                    stmt.allowLoad(), stmt.getReplicaAlloc(), stmt.getTimeoutMs(), metaVersion, stmt.reserveReplica(),
                    stmt.reserveColocate(), stmt.reserveDynamicPartitionEnable(), stmt.isBeingSynced(),
            ...
            restoreJob = new RestoreJob(stmt.getLabel(), stmt.getBackupTimestamp(),
                    ...
                    stmt.isCleanPartitions(), stmt.isAtomicRestore(), stmt.isForceReplace(), stmt.getStorageResource(),
                    stmt.reserveStoragePolicy(), env, repository.getId());
        }
