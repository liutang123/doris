...
if [[ "${RUN_VERSION}" -eq 1 ]]; then
    chmod 755 "${DORIS_HOME}/lib/be/doris_be"
    "${DORIS_HOME}"/lib/be/doris_be --version
    exit 0
fi
...
chmod 550 "${DORIS_HOME}/lib/be/doris_be"
log "Start time: $(date)"
...
if [[ "${RUN_DAEMON}" -eq 1 ]]; then
    if [[ "$(uname -s)" == 'Darwin' ]]; then
        nohup env DYLD_LIBRARY_PATH="${DYLD_LIBRARY_PATH}" ${LIMIT:+${LIMIT}} "${DORIS_HOME}/lib/be/doris_be" "$@" >>"${LOG_DIR}/be.out" 2>&1 </dev/null &
    else
    ...
    ${LIMIT:+${LIMIT}} "${DORIS_HOME}/lib/be/doris_be" "$@" >>"${LOG_DIR}/be.out" 2>&1 </dev/null
fi
