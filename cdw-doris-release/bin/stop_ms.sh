...
if command -v pgrep >/dev/null 2>&1; then
    pgrep_pid="$(pgrep -u doris -x doris_cloud -n)"
    if [[ -z "${pgrep_pid}" ]]; then
        echo "failed to get pid of doris_cloud by pgrep."
    fi
else
...
if test -z "${pid}"; then
    if [[ -z "${pgrep_pid}" ]]; then
        echo "Both the pid of pid file and pgrep pid are invalid pid, maybe doris_cloud not alive."
        exit 0
    else
...
if ! kill -0 "${pid}" 2>&1; then
    ...
    if need_use_pprep_pid; then
        ...
        if ! kill -0 "${pgrep_pid}" 2>&1; then
            echo "ERROR: Either doris_cloud process ${pid} nor ${pgrep_pid} does exist."
            exit 1
        fi
        pid=${pgrep_pid};
    else
        echo "ERROR: doris_cloud process ${pid} does not exist."
        exit 2
    fi
fi
...
if [[ "doris_cloud" != "${pidcomm}" ]]; then
    echo "The pid of pid file indicated process may not doris_cloud MS. "
    if need_use_pprep_pid; then
        echo "Using pgrep pid to continue..."
        pidcomm="$(basename "$(ps -p "${pgrep_pid}" -o comm=)")"
        if [[ "doris_cloud" != "${pidcomm}" ]]; then
            echo "ERROR: pid process may not doris_cloud. "
            exit 3
        fi
	pid=${pgrep_pid};
    else
...
if kill "-${signum}" "${pid}" >/dev/null 2>&1; then
    while true; do
        if kill -0 "${pid}" >/dev/null 2>&1; then
            echo "waiting doris_cloud to stop, pid: ${pid}"
            sleep 2
        else
