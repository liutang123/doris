...
suite("test_backup_cooldown", "backup_cooldown_data") {
    ...
    String repoName = "${suiteName}_repo" + UUID.randomUUID().toString().replace("-", "")
    ...
    def sqlResult = result[0][5].toString();
    int count = 0;
...
// without cold data, use cooldown_datetime
suite("test_backup_cooldown_0", "backup_cooldown_data") {

    String suiteName = "test_backup_cooldown_0"
    ...
    sql """
      ...
      PROPERTIES(
          ...
          "cooldown_datetime" = "2024-12-29 12:00:00"
      )
    """
    ...
    sql """
     ...
     PARTITION BY RANGE(`k1`)
     (
                PARTITION p201701 VALUES [(0), (3)),
                PARTITION `p201702` VALUES LESS THAN (6),
                PARTITION `p2018` VALUES [(6),(100))
    )
     DISTRIBUTED BY HASH (k1) BUCKETS 3
     PROPERTIES(
