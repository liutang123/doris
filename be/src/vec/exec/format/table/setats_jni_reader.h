...
class SetatsJniReader : public GenericReader {
    ENABLE_FACTORY_CREATOR(SetatsJniReader);
public:
    static const std::string HADOOP_CONF_PREFIX;

    SetatsJniReader(const TFileScanRangeParams& scan_params, const TSetatsFileDesc& setats_params,
                    const std::vector<SlotDescriptor*>& file_slot_descs, RuntimeState* state,
                    RuntimeProfile* profile);

    ~SetatsJniReader() override = default;
