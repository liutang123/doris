...
ObjectStorageResponse ObjStorageClient::delete_objects_recursively_(ObjectStoragePathRef path,
    ...
    for (auto obj = list_iter->next(); obj.has_value(); obj = list_iter->next()) {
        ...
        ret = delete_object({.bucket = path.bucket, .key = obj->key});

        if (ret.ret != ObjectStorageResponse::OK && ret.ret != ObjectStorageResponse::NOT_FOUND) {
            LOG(WARNING) << "failed to delete object " << path.bucket << "/" << obj->key
                         << ", ret=" << ret.ret;
            continue;
        }
    }
